/* Progressive enhancement: server-side storyline intelligence for homepage. Games navigation deployment. */
(async()=>{
 ...
 // Keep Local Rally discoverable from the homepage even when the static core nav predates the feature.
 document.querySelectorAll('.newsroom-nav').forEach(nav=>{
  if(![...nav.querySelectorAll('a')].some(a=>(a.textContent||'').trim()==='Local Rally')){
   const local=document.createElement('a');local.href='local/';local.textContent='Local Rally';local.dataset.rpEvent='local_rally_nav_click';
   const games=[...nav.querySelectorAll('a')].find(a=>(a.textContent||'').trim()==='Games');
   if(games)nav.insertBefore(local,games);else nav.appendChild(local);
  }
 });
 ...
 try{
  ...
  try{
   if(!document.querySelector('.latest-brief')){
