...
def importance(all_titles,source_count,age_minutes):
 source_component=min(source_count,6)*3;impact_component=min(sum(weight for phrase,weight in SIGNALS.items() if phrase in all_titles),28);freshness=max(0,1-(max(0,age_minutes)/720))*4
...
def main():
 ...
 for g in cluster(news.get('stories',[])):
  ss=sorted(g['stories'],key=lambda s:s.get('published_epoch') or 0,reverse=True);lead=ss[0];title=lead.get('title','');sources=sorted(x for x in g['sources'] if x);newest=max((s.get('published_epoch') or 0 for s in ss),default=0);age=(now.timestamp()-newest)/60 if newest else 10**9;all_titles=' '.join((s.get('title') or '').lower() for s in ss);risk=sorted({x for x in RISK if x in all_titles});base=importance(all_titles,len(sources),age);identity=' '.join(sorted(g['tokens']))[:300] or title.lower();sid=hashlib.sha1(identity.encode()).hexdigest()[:12]
  prior=history_match(g['tokens'],history);prior_sources=int((prior or {}).get('max_source_count',0));growth=max(0,len(sources)-int((prior or {}).get('current_source_count',0)));persistent=bool(prior and prior_sources>=3);hot_bonus=min(8,max(0,len(sources)-2)*1.5+min(growth,3)*1.5+(2 if persistent and age<=360 else 0));score=round(base+hot_bonus,2)
  status='hot' if len(sources)>=4 and age<=180 else ('developing' if len(sources)>=3 and age<=360 else 'active')
  out.append({'id':sid,'title':title,'importance_score':score,'base_importance_score':base,'hot_bonus':round(hot_bonus,2),'source_count':len(sources),'sources':sources,'newest_epoch':newest,'newest_date':lead.get('date'),'risk_flags':risk,'status':status,'coverage':[{'source':s.get('source'),'title':s.get('title'),'link':s.get('link'),'date':s.get('date')} for s in ss[:8]]})
 out.sort(key=lambda x:(x['importance_score'],x['source_count'],x['newest_epoch']),reverse=True);payload={'generated_at':now.isoformat().replace('+00:00','Z'),'importance_method':'deterministic neutral news-value signals: consequence/institutional significance/safety/economic impact + source breadth + sustained cross-source development; freshness secondary','clustering_method':'distinctive-token overlap plus cluster-core consistency; generic political/news terms excluded to reduce false merges','storyline_count':len(out),'multi_source_count':sum(1 for x in out if x['source_count']>=2),'developing_count':sum(1 for x in out if x['status'] in ('developing','hot')),'hot_count':sum(1 for x in out if x['status']=='hot'),'storylines':out[:500]}
 if OUT.exists():
  try:
 ...
 OUT.parent.mkdir(parents=True,exist_ok=True);OUT.write_text(json.dumps(payload,indent=2,ensure_ascii=False)+'\n');print(f"Built {len(out)} storylines; {payload['multi_source_count']} multi-source; {payload['developing_count']} developing; {payload['hot_count']} hot")
