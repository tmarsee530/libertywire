...
def compatible(t,g):
 # A new headline must agree with the cluster as a whole, not merely one member.
 # This blocks transitive A~B~C chains from pulling unrelated stories together.
...
def cluster(stories):
 ...
 for s in stories:
  ...
  if best:
   ...
   best['core_tokens']=set.intersection(*best['member_tokens']) if best['member_tokens'] else set()
...
def main():
 ...
 for g in cluster(news.get('stories',[])):
  ss=sorted(g['stories'],key=lambda s:s.get('published_epoch') or 0,reverse=True);lead=ss[0];title=lead.get('title','');sources=sorted(x for x in g['sources'] if x);newest=max((s.get('published_epoch') or 0 for s in ss),default=0);age=(now.timestamp()-newest)/60 if newest else 10**9;all_titles=' '.join((s.get('title') or '').lower() for s in ss);risk=sorted({x for x in RISK if x in all_titles});score=importance(all_titles,len(sources),age);identity=' '.join(sorted(g['tokens']))[:300] or title.lower();sid=hashlib.sha1(identity.encode()).hexdigest()[:12]
