...
def main():
    ...
    for item in current.get("storylines",[]):
        ...
        record={"id":sid,"current_title":title,"title_history":titles,"first_seen":prev.get("first_seen") or newest,"last_seen":newest,"max_source_count":max(prev.get("max_source_count",0),item.get("source_count",0)),"current_source_count":item.get("source_count",0),"max_source_family_count":max(prev.get("max_source_family_count",0),item.get("source_family_count",0)),"current_source_family_count":item.get("source_family_count",0),"current_sources":sorted(item.get("sources",[])),"sources":sources,"status":item.get("status","active"),"risk_flags":item.get("risk_flags",[]),"coverage":coverage,"changes":changes[-20:]}
