...
TOPICS={
'government-policy':('Government & Policy','Federal policy, courts, election administration and public agencies.',{'fema','judge','court','usps','ballot','midterms','tariff','policy','federal','government','congress','act','rules','president'}),
'world-markets':('World Affairs & Markets','Geopolitics, trade, energy markets and international shipping.',{'oil','saudi','shipping','red','sea','islands','hormuz','tariff','trade','market','markets','international','pipeline','houthi'}),
}
...
def page(slug,name,dek,briefs):
 ...
 return f'''<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{escape(name)} Explainers | Rally Point News</title><meta name="description" content="Rally Point News source-based explainers: {escape(dek)}"><link rel="canonical" href="https://rallypointnews.com/topics/{slug}/"><style>:root{{--ink:#11100e;--paper:#faf9f6;--navy:#0f1d38;--gold:#c8952c;--rule:#e4e1d8;--muted:#6f6a5e}}*{{box-sizing:border-box}}body{{margin:0;background:var(--paper);color:var(--ink);font-family:Georgia,serif;line-height:1.65}}a{{color:var(--navy)}}header{{background:var(--navy);color:white;border-bottom:3px solid var(--gold);padding:32px 20px}}header div,main{{max-width:820px;margin:auto}}header a{{color:var(--gold)}}h1{{font:800 clamp(32px,6vw,52px)/1.05 Arial,sans-serif;margin:10px 0}}.dek{{color:#d8dfeb}}.brief{{padding:24px 0;border-bottom:1px solid var(--rule)}}.brief h2{{font:800 24px Arial,sans-serif;margin:0 0 8px}}.brief p{{color:var(--muted);margin:0}}main{{padding:36px 20px 70px}}footer{{text-align:center;padding:28px;background:var(--ink);color:#aaa}}footer a{{color:var(--gold)}}</style><script type="application/ld+json">{{"@context":"https://schema.org","@type":"CollectionPage","name":"{escape(name)} Explainers","url":"https://rallypointnews.com/topics/{slug}/"}}</script></head><body><header><div><a href="/topics/">← All Topics</a><h1>{escape(name)}</h1><p class="dek">{escape(dek)}</p></div></header><main>{items}<p style="margin-top:30px"><a href="/briefs/">Browse all Rally Briefs →</a></p></main><footer><a href="/">Rally Point News</a> · <a href="/topics/">Topics</a> · <a href="/sources/">Sources &amp; Methodology</a></footer></body></html>'''
def main():
