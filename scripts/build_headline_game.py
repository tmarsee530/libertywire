...
CANDIDATES=['court','judge','house','votes','voter','trade','storm','plane','press','media','union','money','stock','banks','rates','peace','troop','naval','china','india','japan','crime','trial','order','state','local','mayor','party','polls','power','water','fires','flood','earth','space','virus','drugs','labor','wages','taxes','funds','roads','faith','legal','rules','rights','video','radio','chief','staff','watch']
...
def select_puzzle(storylines):
    ...
    seed=sum(ord(ch) for ch in today_eastern());answer=CANDIDATES[seed%len(CANDIDATES)];story=next((s for s in storylines if not s.get('risk_flags')),{});return answer,story
...
def main():
    ...
    else:answer,story=select_puzzle(storylines)
