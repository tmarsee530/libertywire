...
(async()=>{
 ...
 try{
  ...
  const trust=document.createElement('aside');trust.className='method-note';trust.setAttribute('aria-label','How Rally Point works');trust.innerHTML='<strong>How Rally Point works:</strong> headlines are gathered from participating publishers, grouped into likely storylines, and ranked for recency and cross-source coverage. Multi-source means several publishers are covering the same apparent story; it does not mean Rally Point has independently confirmed every claim. <a href="sources/">See the source roster.</a>';
  nav.insertAdjacentElement('afterend',trust);
