...
def main():
    ...
    for s in data.get('storylines',[]):
        ...
        if flags and source_count<3:continue
        ...
        candidates.append({'storyline_id':s.get('id'),'title':s.get('title'),'priority_score':priority,'importance_score':score,'source_count':source_count,'sources':s.get('sources',[]),'status':s.get('status'),'risk_flags':sorted(flags),'coverage':s.get('coverage',[])[:6],'history':{'first_seen':h.get('first_seen'),'last_seen':h.get('last_seen'),'max_source_count':h.get('max_source_count'),'source_growth':growth},'reason':'multi-source, ranked candidate; requires fresh verification before publication'})
