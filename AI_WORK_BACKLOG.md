...
6. **IN PROGRESS — Visual/UX optimization** — The editorial redesign, mobile/accessibility improvements, trust methodology, and delayed-refresh grace window are installed. Keep the curated newsroom and storyline enhancement on compatible freshness windows so readers do not see an avoidable degraded fallback. Continue measured scannability/placement work once analytics are connected.
...
8. **BLOCKED ON CONNECTION — Analytics-driven optimization** — GA4 and Search Console are not currently exposed through the connected analytics tool. Once connected, measure sessions, landing pages, acquisition sources, engagement, newsletter conversion, search queries, CTR, rankings, and page-level trends, then reorder this backlog from evidence.
