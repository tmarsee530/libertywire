...
def same_publication_event(left,right):
    ...
    lc=event_tokens(clean_title(left.get("current_title"))+" "+ls)
    rc=event_tokens(clean_title(right.get("current_title"))+" "+rs)
    combined_overlap=len(lc&rc); combined_cont=combined_overlap/max(1,min(len(lc),len(rc)))
    return title_overlap>=3 and title_cont>=.55 and combined_overlap>=5 and combined_cont>=.58
...
def page(record, published_records):
    ...
    more_candidates=[x for x in published_records if x.get("id")!=record.get("id") and eligible(x)]
    more_candidates.sort(key=lambda x:parse_dt(x.get("last_seen")),reverse=True)
    more_html="".join(f'<li><a href="/stories/{esc(x.get("id"))}/">{esc(clean_title(x.get("current_title")))}</a></li>' for x in more_candidates[:4])
    more_section=('<section class="related more-live"><h2>More live timelines</h2><ul>'+more_html+'</ul></section>') if more_html else ""
    ...
    schema=json.dumps({"@context":"https://schema.org","@graph":[{"@type":"CollectionPage","@id":f"{BASE}/stories/{record['id']}/#page","name":title,"headline":title,"description":description,"url":f"{BASE}/stories/{record['id']}/","datePublished":record.get("first_seen"),"dateModified":record.get("last_seen"),"author":{"@id":BASE+"/#organization"},"publisher":{"@id":BASE+"/#organization"},"mainEntity":{"@type":"ItemList","numberOfItems":len(schema_items),"itemListOrder":"https://schema.org/ItemListOrderDescending","itemListElement":schema_items},"isPartOf":{"@type":"WebSite","name":"Rally Point News","url":BASE+"/"},"breadcrumb":{"@id":f"{BASE}/stories/{record['id']}/#breadcrumb"}},{"@type":"Organization","@id":BASE+"/#organization","name":"Rally Point News","url":BASE+"/","logo":{"@type":"ImageObject","url":BASE+"/assets/rally-point-news-mark.svg"}},{"@type":"BreadcrumbList","@id":f"{BASE}/stories/{record['id']}/#breadcrumb","itemListElement":[{"@type":"ListItem","position":1,"name":"Rally Point News","item":BASE+"/"},{"@type":"ListItem","position":2,"name":"Live News Timelines","item":BASE+"/stories/"},{"@type":"ListItem","position":3,"name":title,"item":f"{BASE}/stories/{record['id']}/"}]}]},ensure_ascii=False).replace("</","<\\/")
    ga=analytics_tag()
    return f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{esc(title)} — Live Timeline | Rally Point News</title><meta name="description" content="{esc(description)}"><meta name="robots" content="index,follow,max-snippet:-1,max-image-preview:large"><link rel="canonical" href="{BASE}/stories/{sid}/"><link rel="alternate" type="application/rss+xml" title="Rally Point News — Live Timelines" href="{BASE}/feed.xml">{ga}<meta property="og:type" content="website"><meta property="og:site_name" content="Rally Point News"><meta property="og:title" content="{esc(title)} — Live Timeline"><meta property="og:description" content="{esc(description)}"><meta property="og:url" content="{BASE}/stories/{sid}/"><meta property="og:image" content="{BASE}/assets/rally-point-news-mark.svg"><meta property="og:image:alt" content="Rally Point News"><meta property="article:published_time" content="{esc(record.get('first_seen'))}"><meta property="article:modified_time" content="{esc(record.get('last_seen'))}"><meta name="twitter:card" content="summary"><meta name="twitter:image" content="{BASE}/assets/rally-point-news-mark.svg"><meta name="twitter:title" content="{esc(title)} — Live Timeline"><meta name="twitter:description" content="{esc(description)}"><link rel="stylesheet" href="/assets/timeline.css?v=4"><script type="application/ld+json">{schema}</script></head><body><a class="skip-link" href="#timeline">Skip to updates</a><header><a class="mast" href="/">Rally Point News</a><nav><a href="/">Top Stories</a><a href="/#grid">The Wire</a><a href="/stories/">Live Timelines</a><a href="/sources/">Sources</a></nav></header><main><nav class="breadcrumbs" aria-label="Breadcrumb"><a href="/">Home</a><span>›</span><a href="/stories/">Live Timelines</a></nav><p class="status">{esc(status)}</p><h1>{esc(title)}</h1><p class="dek">A concise, source-backed record of what materially changed.</p><p class="byline">By <a href="/about/">Rally Point News</a> · AI-assisted aggregation with source attribution</p><div class="story-meta"><span>Updated {esc(display_time(record.get('last_seen')))}</span><span>Tracking since {esc(display_time(record.get('first_seen')))}</span><span>{len(updates)} material developments</span><span>{len(source_names)} sources</span></div><section class="current-status" aria-labelledby="current-status-heading"><p class="status-kicker">{esc(current.get('classification') or 'CURRENT STATUS')}</p><h2 id="current-status-heading">Current status</h2><p class="status-summary">{esc(summary)}</p><p class="status-source"><time datetime="{esc(current.get('as_of'))}">As of {esc(display_time(current.get('as_of')))}</time>{' · '+current_source if current_source else ''}</p></section><div class="caught-up"><strong>NEWEST FIRST</strong><span>Eastern Time · material updates only</span></div><ol class="timeline" id="timeline" aria-label="Latest developments, newest first">{recent_html}</ol>{earlier_html}<div class="end-card"><strong>You’ve reached the beginning.</strong><span>That’s the earliest material development tracked in this story.</span></div><section class="sources"><h2>Sources tracking this story</h2><p>{esc(' · '.join(source_names))}</p><p><a href="/about/">How Rally Point selects, groups, attributes, and corrects timeline coverage →</a></p></section>{related_section}{more_section}<p class="back"><a href="/">← Back to Rally Point News</a></p></main><footer>Rally Point News · Headlines and reporting belong to their respective publishers. <a href="/about/">Editorial standards</a></footer></body></html>'''
